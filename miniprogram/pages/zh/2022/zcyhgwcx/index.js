...
Page({
     ...
     */
    data: {
        title:"小程序-2023政策性银行秋招职位查询",
        imageUrl:"http://jl.offcn.com/zg/ty/images/exam-helper-mini-program/zh/zcyh/zcyh-share.jpg",
        CRMEFSID: "5793d4ab08ffd949b56d8b59442b7663", // CRM 活动表单 ID
        CRMRemark: "活动编码:HD202206270665,活动表单ID:133475", // CRM 注释  2023人民银行秋招职位查询
        actid: "48977", //zg99id  
    ...
    m_select_touch(e) {
        ...
        switch (e.detail.type) {
            ...
            case "city": //地市
                ...
                wx.request({
                    ...
                    success(res) {
                        ...
                        // 将数据添加到已清空省份
                        for (var i = 0; i < response.lists.length; i++) {
                            List.push(response.lists[i].item5)
                        }
                        // console.log(List);
                        _this.setData({
                            xueliList: List
            ...
            case "xueli": //学历
                _this.setData({
                    xueliValue: _this.data.xueliList[e.detail.index],
                    cetList: []
                })
                // zg99六级联动
                wx.request({
                    ...
                    data: {
                        level: '6',
                        grfiled: 'item5',
                        grtext: _this.data.xueliList[e.detail.index],
                        onefiled:'item1',
                        onetext:this.data.yearValue,
                        twofiled:'item2',
                        twotext:this.data.bankValue,
                    ...
                    success(res) {
                        ...
                        // 将数据添加到已清空省份
                        for (var i = 0; i < response.lists.length; i++) {
                            List.push(response.lists[i].item6)
                        }
                        // console.log(List);
                        _this.setData({
                            cetList: List
    ...
    search() {
        ...
        wx.request({
            ...
            data: {
                ...
                item5: this.data.xueliValue,
                item6: this.data.cetValue,
                limits: 200,
                page: this.data.page
            },
            success(res) {
    ...
    onReachBottom: function () {
        if (this.data.yearValue !== "" && this.data.bankValue !== "" && this.data.provenceValue !== "" && this.data.cityValue !== "" && this.data.xueliValue !== "" && this.data.cetValue !== "" && this.data.result.length < this.data.count) {
            this.search();
        }
    },
