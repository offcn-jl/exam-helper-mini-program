...
Page({
  data: {
    ...
    superiorLink:"/pages/match-2022-ybjh/index", //上级链接
    actid:"5394", //zg99id  和前面的不是一个zg99id了
  ...
  onLoad: function (options) {
    ...
    if (typeof options.pro !== "undefined" && options.pro !== "不限") this.setData({ "query.pro": options.pro })
    if (typeof options.address !== "undefined" && options.address !== "不限") this.setData({ "query.address": options.address })
    if (typeof options.area !== "undefined" && options.area !== "不限") this.setData({ "query.area": options.area })
    if (typeof options.position !== "undefined" && options.position !== "不限") this.setData({ "query.position": options.position })
    this.search();
  },
  /**
   * 生命周期函数--监听页面初次渲染完成
