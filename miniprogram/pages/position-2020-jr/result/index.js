...
Page({
  ...
  search(){
    ...
    wx.request({
      url: "https://zg99.offcn.com/index/chaxun/getfylist?actid=29869&callback=?",
  ...
  onLoad: function (options) {
    ...
    // 配置查询条件
    if (typeof options.school !== "undefined" && options.school !== "不限") this.setData({ "query.school": options.school })
    if (typeof options.education !== "undefined" && options.education !== "不限") this.setData({ "query.education": options.education })
    ...
    if (typeof options.graduates !== "undefined" && options.graduates !== "不限") this.setData({ "query.graduates": options.graduates })
    if (typeof options.city !== "undefined" && options.city !== "不限") this.setData({ "query.city": options.city })
    if (typeof options.bank !== "undefined" && options.bank !== "不限") this.setData({ "query.bank": options.bank })
    this.search();
  },
  /**
   * 生命周期函数--监听页面初次渲染完成
  ...
  onShareAppMessage: function () {
    return {
      title: '2021银行春季校园招聘职位筛选',
      path: "/pages/position-2020-jr/index?scene=" + this.data.suffix,
