...
Page({
    ...
    login: function (event) {
        ...
        getApp().methods.requsetWithCode({
            ...
            callback: res => {
                this.setData({ phone: res.data.phone, openid: res.data.openid });
                // 弹出 Loading
                wx.showLoading({ title: '请稍候...', mask: true });
                // 保存注册记录到数据库
                wx.cloud.database().collection('user2022sk').add({
                    data: { phone: res.data.phone, openid: res.data.openid, suffix: this.data.suffix, createdTime: new Date() }
    ...
    onLoad: function (options) {
        ...
        if (wx.getLaunchOptionsSync().scene !== 1154) {
            ...
            getApp().methods.requsetWithCode({
                ...
                callback: res => {
                    if (res.errorMessage !== '用户未登录或登陆态已失效') {
                        _this.setData({ phone: res.data.phone, openid: res.data.openid });
                        setTimeout(() => { wx.pageScrollTo({ selector: '.doc-title', duration: 1000 }) }, 1000);
                        // 弹出 Loading
                        wx.showLoading({ title: '请稍候...', mask: true });
                        // 保存注册记录到数据库
                        wx.cloud.database().collection('user2022sk').add({
                            data: { phone: res.data.phone, openid: res.data.openid, suffix: options.scene, createdTime: new Date() }
                        }).then(collectionAddRes => {
                            if (collectionAddRes.errMsg == 'collection.add:ok') {
                                // 操作成功
                                wx.hideLoading(); // 隐藏 loading
