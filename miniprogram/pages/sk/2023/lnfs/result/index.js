...
Page({
  data: {
    title:"2023省考历年分数线查询",// 标题
    imageUrl:"https://jl.offcn.com/zt/ty/images/sk/2023/lnfs/share.jpg",// 分享时显示的图片
    superiorLink:"/pages/sk/2023/lnfs/index", //上级链接
  ...
  lnfsx: function (event) {
    ...
    let _this = this //作用域 
    wx.request({
      url: "https://zw.offcn.com/wechat/zhiwei/lnfsx?sign=b4a8b2f7z4d3n6o0hs",
      method: "post",
      header: {
        "Content-Type": "application/x-www-form-urlencoded"
      },
      data: {id:event.currentTarget.dataset.id,zwcode:'jl',zwyear:this.data.query.zwyear,sstimes: new Date().getTime()},
      success(res) {
        wx.hideLoading() // 隐藏 loading
        try {
          let list = res.data
