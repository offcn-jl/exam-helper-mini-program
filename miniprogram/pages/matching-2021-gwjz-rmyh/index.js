Page({
  data: {
    ...
    CRMEFSID: "69f2bac2c2bc1000bdadc92b26da759a", // CRM 活动表单 ID
    CRMEventID: "HD202112310212", // CRM 注释  小程序-2022人民银行岗位竞争比,活动表单ID:95638

    nfList: ['2022','2021','2020','2019'],//年份
    cityList:['吉林','北京','天津','上海','重庆','河北','山西','内蒙古','辽宁','黑龙江','江苏','浙江','安徽','福建','江西','山东','河南','湖北','湖南','广东','广西','海南','四川','贵州','云南','西藏','陕西','甘肃','青海','宁夏','新疆'], // 城市
  ...
  tipsToSubscribeMessage() {
    ...
    if (!_this.data.tipsToSubscribeMessaged) {
      ...
      wx.showModal({
        title: '提示',
        content: '您是否需要订阅“银行”考试公告？订阅成功后您可以在公告发布时免费获得推送提示～',
        confirmText: "免费订阅",
        success(res) {
          if (res.confirm) {
            getApp().methods.subscribeSingleExam(_this.data.suffix, "银行", undefined, () => {
